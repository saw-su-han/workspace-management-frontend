...
export const DashboardLayout: React.FC = () => {
    ...
    if (isProfileLoading) {
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-slate-50 dark:bg-[#090d16] gap-6 transition-colors duration-300 relative overflow-hidden font-sans">
                <FontFaces />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(11,244,200,0.08)_0%,_transparent_60%)]" />
                ...
                <div className="relative flex h-20 w-20 items-center justify-center">
                    ...
                    <div className="relative inline-flex rounded-2xl h-11 w-11 bg-gradient-to-tr from-mint-600 via-teal-500 to-emerald-400 shadow-xl shadow-mint-500/30 items-center justify-center text-slate-950 font-bold glow-mint">
                        <Icon icon="lucide:hexagon" className="w-6 h-6 animate-pulse" />
                    </div>
                </div>
                <div className="space-y-1 text-center relative px-4">
                    <p className="font-display text-base font-bold tracking-tight text-slate-900 dark:text-white">Loading workspaces...</p>
                    <p className="font-mono-nav text-[10px] text-mint-600/80 dark:text-mint-400/80 uppercase tracking-widest">Synchronizing state node</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#090d16] text-slate-900 dark:text-slate-100 transition-colors duration-300 flex flex-col antialiased relative overflow-x-hidden font-sans">
            ...
            <div
                ...
                style={{
                    backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(11, 244, 200, 0.2) 1px, transparent 0)',
                    backgroundSize: '28px 28px'
                }}
            />
            {/* Atmospheric Background Glows */}
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-mint-500/10 dark:bg-mint-500/10 blur-[160px] rounded-full pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-500/10 dark:bg-teal-500/10 blur-[160px] rounded-full pointer-events-none" />

            {/* HEADER - RESPONSIVE MOBILE & DESKTOP */}
            <header className="h-16 md:h-20 border-b border-slate-200/80 dark:border-slate-800/80 bg-white/75 dark:bg-[#090d16]/75 backdrop-blur-2xl px-4 md:px-8 sticky top-0 z-40 transition-colors flex items-center justify-between gap-3 shadow-lg shadow-slate-950/5">
                <div className="flex items-center gap-3.5 flex-shrink-0">
                    <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-mint-600 via-teal-500 to-emerald-400 text-slate-950 shadow-lg shadow-mint-500/20 glow-mint">
                    ...
                    <div>
                        <h1 className="font-display font-extrabold text-sm md:text-base tracking-tight text-slate-900 dark:text-white flex items-center gap-1.5">
                            Project<span className="text-mint-600 dark:text-mint-400">Hive</span>
                        </h1>
                        <p className="font-mono-nav text-[9px] font-semibold text-slate-400 dark:text-slate-400 tracking-widest uppercase -mt-0.5">Workspace Console</p>
                    </div>
                </div>
                        ...
                        {filteredWorkspaces.map((ws: any) => {
                            ...
                            return (
                                <div
                                    ...
                                    className="group relative flex flex-col justify-between p-6 rounded-3xl bg-white/90 dark:bg-gray-900/70 border border-gray-200/80 dark:border-gray-800/80 shadow-xl shadow-black/[0.03] hover:shadow-2xl hover:-translate-y-1 hover:border-mint-500/60 dark:hover:border-mint-500/60 transition-all duration-300 cursor-pointer overflow-hidden backdrop-blur-2xl"
                                >
                                    {/* Top accent glowing line */}
                                    <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-mint-500 via-teal-500 to-emerald-400 opacity-80 group-hover:opacity-100 transition-opacity duration-300" />
                                    <div className="absolute top-0 right-0 w-36 h-36 bg-mint-500/10 dark:bg-mint-500/10 rounded-full blur-2xl pointer-events-none group-hover:bg-mint-500/20 transition-all" />
                                    ...
                                    <div className="flex items-center justify-between gap-3 relative z-10">
                                        <div className="flex items-center gap-3.5">
                                            <UserAvatar
                                                ...
                                                className="w-12 h-12 rounded-2xl object-cover border border-gray-200 dark:border-gray-700 shadow-md group-hover:scale-110 group-hover:border-mint-500/40 transition-all duration-300"
                                            />
                                            <div>
                                                <h4 className="font-display font-extrabold text-base text-gray-900 dark:text-white group-hover:text-mint-600 dark:group-hover:text-mint-400 transition-colors line-clamp-1">
                                                    {workspaceInfo.name}
                                                </h4>
                                                <span className="font-mono-nav text-[11px] font-semibold text-gray-400 dark:text-gray-500 tracking-wider flex items-center gap-1.5 mt-0.5">
                                        ...
                                        {/* Clean Glowing Status Indicator */}
                                        <span className="w-3 h-3 rounded-full bg-gradient-to-tr from-mint-500 to-teal-400 ring-4 ring-mint-500/20 flex-shrink-0 animate-pulse" />
                                    </div>

                                    {/* Card Footer / Details */}
                                    <div className="flex items-center justify-between pt-6 mt-6 border-t border-gray-100 dark:border-gray-800/80 relative z-10">
                                        <span className={`font-mono-nav text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1.5 shadow-xs ${roleText === 'OWNER'
                                            ? 'bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/20'
                                            : roleText === 'ADMIN'
                                                ? 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20'
                                                : 'bg-gray-100 dark:bg-gray-800/80 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700'
                                            }`}>
                                            <Icon icon={roleText === 'OWNER' ? 'lucide:shield-check' : roleText === 'ADMIN' ? 'lucide:shield' : 'lucide:user'} className="w-3.5 h-3.5" />
                                            {roleText}
                                        </span>

                                        <div className="flex items-center gap-1.5 font-mono-nav text-xs font-extrabold text-mint-600 dark:text-mint-400 group-hover:translate-x-1.5 transition-transform">
