                ...
                <header className="w-full max-w-7xl mx-auto flex items-center justify-between py-3 px-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/75 dark:bg-[#090d16]/75 backdrop-blur-2xl shadow-xl shadow-slate-950/5">
                    ...
                    <nav className="hidden md:flex items-center gap-1.5 font-code text-xs font-medium bg-gray-100 dark:bg-gray-800 p-1 rounded-lg border border-gray-200 dark:border-gray-700">
                        {['Workflow', 'Features', 'Architecture', 'Security'].map(item => (
                            <a key={item} href={`#${item.toLowerCase()}`} className="px-4 py-1.5 rounded-md text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-gray-900 hover:text-mint-600 dark:hover:text-mint-400 transition-all">
                                {item}
                            </a>
                        ))}
                    </nav>
                    ...
                    <div className="flex items-center gap-3">
                        <ThemeToggle />
                        <Link to="/login" className="font-code text-xs font-medium text-gray-600 dark:text-gray-300 hover:text-mint-600 dark:hover:text-mint-400 px-3 py-2">
                            Sign In
                        </Link>
                        <Link to="/register" className="font-code rounded-xl bg-mint-600 hover:bg-mint-500 text-white px-5 py-2.5 text-xs font-bold uppercase tracking-wider transition-all shadow-md active:scale-[0.98]">
                            Sign Up
                        </Link>
                    </div>
                </header>
            ...
            <main className="flex flex-col items-center w-full max-w-7xl mx-auto z-10 pt-32 pb-24 px-4 space-y-32">
                ...
                <section className="grid md:grid-cols-5 gap-12 items-center w-full py-10">
                    <div className="md:col-span-3 space-y-8 text-left relative">
                        <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-mint-500/10 dark:bg-mint-950/60 border border-mint-500/20 font-code text-xs font-medium text-mint-600 dark:text-mint-400">
                        ...
                        <h1 className="font-display text-5xl md:text-7xl font-extrabold tracking-tighter text-gray-950 dark:text-white leading-[0.95]">
                            ...
                            <span className="text-mint-600 dark:text-mint-400">engineering swarms.</span>
                        </h1>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-4">
                            <Link to="/register" className="font-code inline-flex items-center gap-2 rounded-2xl bg-mint-600 hover:bg-mint-500 text-white px-8 py-4 text-sm font-bold uppercase tracking-widest transition-all shadow-lg shadow-mint-600/20 active:scale-[0.98]">
                                Initialize Hub <Icon icon="lucide:zap" className="w-4 h-4" />
                            </Link>
                            <Link to="/architecture" className="font-code text-mint-600 dark:text-mint-400 text-sm font-semibold px-6 py-4 hover:underline">
                                View System Specs →
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 bg-gray-950 dark:bg-gray-900 p-5 rounded-3xl shadow-2xl shadow-black/20 border border-gray-700 font-code text-xs relative overflow-hidden group">
                        ...
                        <div className="space-y-1.5 text-gray-300">
                            <div><span className="text-gray-500">1</span> <span className="text-mint-400">user@hive:~/$</span> projecthive init --org <span className="text-yellow-300">"CoreLogic"</span></div>
                            <div><span className="text-gray-500">2</span> <span className="text-mint-400">user@hive:~/$</span> <span className="text-blue-400">✔</span> Workspace <span className="text-yellow-300">'core-prod-alpha'</span> created.</div>
                            <div><span className="text-gray-500">3</span> <span className="text-mint-400">user@hive:~/$</span> hive members add --email <span className="text-yellow-300">lead@core.io</span> --role <span className="text-yellow-300">admin</span></div>
                            <div><span className="text-gray-500">4</span> <span className="text-mint-400">user@hive:~/$</span> <span className="text-blue-400">✔</span> Member invited successfully.</div>
                            <div><span className="text-gray-500">5</span> <span className="text-mint-400">user@hive:~/$</span> hive project create --name <span className="text-yellow-300">"QuantumLeap"</span> --lead <span className="text-yellow-300">@thompson</span></div>
                            <div><span className="text-gray-500">6</span> <span className="text-mint-400">user@hive:~/$</span> <span className="text-blue-400">✔</span> Project <span className="text-yellow-300">QuantumLeap</span> initialized.</div>
                            <div><span className="text-gray-500">7</span> <span className="text-mint-400">user@hive:~/$</span> <span className="animate-pulse">█</span></div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-200 dark:divide-gray-800">
                        {[
                            ...
                            <div key={metric.label} className="text-center px-4 first:pl-0 last:pr-0">
                                <div className="font-display text-4xl text-mint-600 dark:text-mint-400 tabular-nums">{metric.value}</div>
                    ...
                    <div className="relative flex flex-col lg:flex-row justify-between items-center gap-8 w-full max-w-5xl mx-auto">
                        ...
                        {[
                            ...
                            <div key={step.num} className="relative z-10 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-8 rounded-3xl shadow-xl flex flex-col justify-between w-full max-w-xs transition-transform hover:-translate-y-1">
                                <div>
                                    <div className="font-code text-xs font-bold text-mint-600 dark:text-mint-400 mb-2">PHASE // {step.num}</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-2 p-8 rounded-3xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 flex flex-col justify-between relative overflow-hidden group">
                            <div className="space-y-4 max-w-md relative z-10">
                                <div className="w-10 h-10 rounded-2xl bg-mint-500/10 flex items-center justify-center text-mint-600 dark:text-mint-400">
                            ...
                            <div className="absolute right-0 bottom-0 translate-x-8 translate-y-8 w-64 h-64 bg-mint-500/10 rounded-full blur-2xl pointer-events-none"></div>
                        </div>
                        ...
                        <div className="md:col-span-2 p-8 rounded-3xl bg-gradient-to-r from-teal-950 via-mint-950 to-gray-950 text-white flex flex-col justify-between relative overflow-hidden border border-mint-500/20">
                            <div className="space-y-4 max-w-md relative z-10">
                                <h3 className="font-display text-2xl font-bold">Absolute Data Sovereignty</h3>
                                <p className="text-sm text-gray-300">Every workspace maintains cryptographically secure separation with end-to-end audit logging for internal governance.</p>
                            </div>
                            <Link to="/register" className="mt-6 inline-flex font-code text-xs font-bold uppercase tracking-wider text-mint-950 bg-mint-400 hover:bg-mint-300 px-5 py-3 rounded-xl w-fit transition-colors">
                ...
                <section id="architecture" className="w-full space-y-12 scroll-mt-24">
                    <header className="text-center max-w-xl mx-auto space-y-3">
                        <span className="font-code text-xs font-bold tracking-widest uppercase text-mint-600 dark:text-mint-400">System Topology</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="p-8 rounded-3xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 space-y-4">
                            ...
                            <div className="font-code text-xs text-mint-600 dark:text-mint-400 pt-2 flex items-center gap-2">
                ...
                <section id="security" className="w-full space-y-12 scroll-mt-24">
                    <header className="text-center max-w-xl mx-auto space-y-3">
                        <span className="font-code text-xs font-bold tracking-widest uppercase text-mint-600 dark:text-mint-400">Zero Trust Framework</span>
